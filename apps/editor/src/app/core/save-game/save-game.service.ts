    ...
    reader.addEventListener('loadend', async (event) => {
      try {
        ...
        if (target) {
          ...
          console.log('Root properties:', Object.keys(binarySave?.root?.properties || {}));
          console.log('saveData_0 structure:', binarySave?.root?.properties?.saveData_0);
          console.log('SaveData_0 structure:', binarySave?.root?.properties?.SaveData_0);
          
          // Try to access the save data with multiple possible paths (handle different save versions)
          ...
          console.log(`Using save data key: ${saveDataKey}`);
          console.log('Save data root:', saveDataRoot);
          
          const saveData = saveDataRoot?.Struct?.value?.Struct ||
                           saveDataRoot?.value?.Struct ||
                           saveDataRoot;
          console.log('SaveData structure:', saveData);
        }
      } catch (e) {
